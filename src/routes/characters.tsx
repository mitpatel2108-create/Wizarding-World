  ...
  head: () => ({
    meta: [
      { title: "Wizards & Witches — Wizarding World" },
      {
        name: "description",
        content:
          "Meet the wizards and witches whose portraits stir at midnight. Each carries a story older than the stones.",
      },
...
const CHARACTERS: Character[] = [
  {
    name: "The Headmaster - Albus Dumbledore",
    role: "Sorcerer Supreme of the Castle",
    house: "Gryffindor",
    image: headmasterImg,
  ...
  {
    name: "The Deputy - Minerva McGonagall",
    role: "Mistress of Transfiguration",
    house: "Gryffindor",
    image: deputyImg,
  ...
  {
    name: "The Potions Master - Professor Severus Snape",
    role: "Keeper of the Dungeons",
    house: "Slytherin",
    image: potionsImg,
  ...
  {
    name: "The Keeper of Keys - Rubeus Hagrid",
    role: "Gamekeeper of the Forbidden Forest",
    house: "Gryffindor",
    image: keeperImg,
    quote: "Yer a wizard.",
  ...
  {
    name: "The Dreamer - Luna Lovegood",
    role: "Reader of Invisible Things",
    house: "Ravenclaw",
    image: dreamerImg,
  ...
  return (
    <section className="relative min-h-screen overflow-hidden pt-32">
      ...
      <div className="relative mx-auto max-w-6xl px-6 text-center">
        <p className="font-display text-xs uppercase tracking-[0.5em] text-[var(--gold)]/70">
          The Portraits
        </p>
        ...
        <p className="mx-auto max-w-2xl font-serif-magical text-xl italic text-[oklch(0.86_0.04_75)]/85">
          The frames stir when no one watches. Each portrait remembers a name, a wand, a kindness, a
          wound.
        </p>
      </div>
      <div className="relative mx-auto mt-20 grid max-w-7xl gap-10 px-6 pb-24 sm:grid-cols-2 lg:grid-cols-3">
        {CHARACTERS.map((c) => (
          <PortraitCard key={c.name} character={c} />
        ))}
      </div>
    </section>
  );
    ...
    >
      ...
      <img
        ...
        style={{
          ...
          filter: open
            ...
            : "saturate(0.85) brightness(0.85) contrast(1.1)",
        }}
      />
      <div
        ...
        style={{
          background:
            "radial-gradient(ellipse at center, transparent 50%, rgba(0,0,0,0.55) 100%), linear-gradient(180deg, transparent 50%, rgba(5,5,15,0.95) 100%)",
        }}
      />
      {open && <Particles variant="spark" count={20} />}
      <div
        className="absolute left-1/2 top-1/4 h-2 w-2 -translate-x-1/2 rounded-full transition-opacity duration-700"
        style={{
          background: character.glow,
          boxShadow: `0 0 30px 8px ${character.glow}`,
          opacity: open ? 0.9 : 0,
      ...
      <div className="absolute inset-x-0 bottom-0 z-10 p-6">
        <p
          ...
          style={{ color: character.glow }}
        >
          {character.house}
        </p>
        <h3 className="mt-2 font-display text-2xl text-[var(--gold)]">{character.name}</h3>
        <p className="font-serif-magical text-sm italic text-[oklch(0.85_0.04_75)]/80">
          {character.role}
        </p>
        <div
        ...
        >
          <div className="overflow-hidden">
            <p
              className="mt-4 font-serif-magical text-base italic"
              style={{ color: character.glow }}
            >
              "{character.quote}"
            </p>
            <p className="mt-2 font-serif-magical text-sm italic text-[oklch(0.85_0.04_75)]/70">
              {character.bio}
            </p>
          </div>
        </div>
