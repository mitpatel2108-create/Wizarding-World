...
export const Route = createFileRoute("/profile")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Your house, your wand, your patronus, your accumulated magic.",
      },
  ...
  return (
    <section className="relative min-h-screen overflow-hidden pt-32 pb-24">
      ...
      <div className="relative mx-auto mt-8 max-w-4xl px-6">
        <article
          className="relative overflow-hidden border border-[var(--gold)]/40 p-10 shadow-arcane md:p-14"
          style={{
            background: house
        ...
        >
          <div className="grid gap-10 md:grid-cols-[auto_1fr] md:items-start">
            ...
            <div className="flex flex-col items-center">
              <div className="relative h-40 w-40">
                <div
                  ...
                  style={{
                    background: house
                      ? `radial-gradient(circle, ${house.glow}55, transparent 70%)`
              ...
              <p
                className="mt-4 font-display text-[10px] uppercase tracking-[0.4em]"
                style={{ color: house?.glow ?? "var(--gold)" }}
              >
                {house?.name ?? "Unsorted"}
              </p>
            ...
            {/* Details */}
            <div>
              <label className="font-display text-[10px] uppercase tracking-[0.4em] text-[var(--gold)]/70">
                Witch / Wizard
              </label>
              <input
                value={p.name}
              ...
              <dl className="mt-8 grid grid-cols-1 gap-6 sm:grid-cols-2">
                <Stat
                  label="House"
                  value={house?.name ?? "Unsorted"}
                  link={house ? null : "/sorting-hat"}
                  linkLabel="Be sorted"
                />
                <Stat
                  label="Wand"
                  value={p.wand ?? "Unchosen"}
                  link={p.wand ? null : "/wands"}
                  linkLabel="Choose a wand"
                />
                <Stat
                  label="Patronus"
                  value={p.patronus ?? "Unrevealed"}
                  link={p.patronus ? null : "/patronus"}
                  linkLabel="Cast Patronus"
                />
              ...
              {mounted && (
                <div className="mt-10">
                  <p className="mb-3 font-display text-[10px] uppercase tracking-[0.4em] text-[var(--gold)]/70">
                    Quick Sort (preview)
                  </p>
...
function Stat({
  ...
  return (
    <div>
      <dt className="font-display text-[10px] uppercase tracking-[0.4em] text-[var(--gold)]/60">
        {label}
      </dt>
      <dd className="mt-2 font-serif-magical text-2xl italic text-[oklch(0.92_0.05_85)]">
        {value}
      </dd>
      {link && (
        <Link
          to={link}
          className="mt-1 inline-block font-display text-[10px] uppercase tracking-[0.3em] text-[var(--gold)]/80 underline-offset-4 hover:underline"
        >
          {linkLabel} →
        </Link>
      )}
